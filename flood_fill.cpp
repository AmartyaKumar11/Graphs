...
plus any pixels connected 4-directionally to the starting pixel of the same colour as the starting pixel, 
...
We can use any of the traversals to solve this problem, in our case we will be using DFS just to explore multiple approaches. 
...
It is advised to use another matrix because we work on the data and not tamper with it. So we will create a copy of the input matrix. 
Check for the neighbours of the respective pixel that has the same initial colour and has not been visited or coloured. 
DFS call goes first in the depth on either of the neighbours.
...
From the above image, it is clear that delta change in row is -1, +0, +1, +0. 
Similarly, the delta change in column is 0, +1, +0, -1.  So we can apply the same logic to find the neighbours of a particular pixel (<row, column>). 
*/
